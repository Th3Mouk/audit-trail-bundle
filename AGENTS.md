...
Everything in the design serves one of those two sentences, or the invariant that makes them
...
| `src/Metadata/` | Attribute reading, memoised: `AuditableResolver`, `FieldPolicyResolver`, `FieldPolicy` | Reflection belongs here and nowhere else |
...
| Change the `audit_logs` shape | Entity + `docs/installation.md`'s SQL block + a `CHANGELOG.md` note. It is a major version: applications own their migration |
