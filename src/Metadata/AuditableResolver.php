...
final class AuditableResolver
{
    ...
    public function isAuditable(string $class): bool
    {
        ...
        $enabled = $this->isEnabledFor($class);
    ...
    /**
     * Whether the class opted in, without asking whether the bundle can actually audit it.
     *
     * {@see isAuditable()} answers both, and is what capture asks. This one exists for callers that
     * must not raise on an unsupported entity — a cache warmer inspecting every mapped class would
     * otherwise turn a composite-key entity into a boot failure, which is a flush-time concern.
     *
     * @param class-string $class
     */
    public function isEnabledFor(string $class): bool
    {
        return $this->enabled[$class] ??= $this->readAuditableFlag($class);
    }
