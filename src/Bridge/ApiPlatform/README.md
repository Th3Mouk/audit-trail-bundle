...
| `audit_trail.bridges.api_platform.enabled` | auto-detect |
| `audit_trail.bridges.api_platform.route_prefix` | `/audit_logs` |
| `audit_trail.bridges.api_platform.items_per_page` | `50` |
| `audit_trail.bridges.api_platform.max_items_per_page` | `200` |
| `audit_trail.bridges.api_platform.security` | none emitted |

Auto-detection is `interface_exists(ResourceMetadataCollectionFactoryInterface::class)` plus the
...
| `…filter.exact` | `SearchFilter` | `actorType`, `actorId`, `entityClass`, `entityId`, `rootType`, `rootId` |
...
| `…filter.order` | `OrderFilter` | `id`, `occurredAt` |
...
`security` is emitted only when `bridges.api_platform.security` is non-null. The bundle never
invents a permission name: `"is_granted('ROLE_ADMIN')"` and friends are the host's vocabulary. With
no expression configured, the resource metadata carries no security attribute at all and the feed is
as protected as the host's firewall and `access_control` make it.
