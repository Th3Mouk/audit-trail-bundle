...
final readonly class DefaultScopeResolver implements ScopeResolverInterface
{
    ...
    public function resolve(object $entity): ?AuditRef
    {
        if ($entity instanceof AuditScopeProviderInterface) {
            return $this->normalised($entity->resolveAuditRoot());
        }
        ...
        return AuditRef::of(
            $scope->type ?? $this->typeResolver->typeOf($this->classOf($root)),
            $id,
            $this->labelResolver->resolve($root),
        );
    }

    /**
     * The same type {@see resolve()} would put in its reference, without walking to the root.
     *
     * Callers that already hold the reference should read its `type`; this exists for the ones that
     * only need to know which aggregate kind an entity belongs to.
     */
    public function resolveType(object $entity): ?string
    {
        if ($entity instanceof AuditScopeProviderInterface) {
            return $this->normalised($entity->resolveAuditRoot())?->type;
        }
        ...
        return $scope->type ?? $this->typeResolver->typeOf($scope->root);
    }
    ...
    /**
     * An entity naming its own root may hand over either a type or the class it has to hand;
     * both arrive here as the type the trail stores.
     */
    private function normalised(?AuditRef $root): ?AuditRef
    {
        return null === $root
            ? null
            : AuditRef::of($this->typeResolver->typeOf($root->type), $root->id, $root->label);
    }
}
