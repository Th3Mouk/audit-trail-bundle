...
final readonly class AuditLogListener
{
    public function __construct(
        ...
        private EntityManagerInterface $auditedEntityManager,
    ...
    public function onFlush(OnFlushEventArgs $args): void
    {
        ...
        // Doctrine registers a listener on every connection unless the tag names one, so a flush of
        // a second entity manager reaches this method too. Capturing it would enlist the entry in a
        // different UnitOfWork from the change it describes — outside its transaction, which is the
        // one guarantee this bundle makes. One manager owns the trail; the others are not audited.
        if ($entityManager !== $this->auditedEntityManager) {
            return;
        }
    ...
    private function buildEntry(
        ...
        return new AuditEntry(
            $action,
            $this->typeResolver->typeOf($entityClass),
    ...
    private function resolveRoot(object $entity): ?AuditScopeRef
    {
        ...
        return AuditScopeRef::of($root->type, $root->id, $root->label);
    }
}
