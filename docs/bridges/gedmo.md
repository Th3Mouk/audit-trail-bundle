...
audit_trail:
    bridges:
        gedmo:
            enabled: ~              # null = auto-detect the package
            translatable: true      # translation auditing, and the field trimming that goes with it
            soft_deleteable: true   # a logical delete reads as a delete
            listener_priority: 1    # the translation listener, which must stay above Gedmo's own
```
