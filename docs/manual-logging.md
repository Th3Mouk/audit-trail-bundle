...
## Naming the thing that changed

The first argument is the **type** the entry is filed under, and it takes either form:

```php
// An entity: converted to the type capture would have used, declared or derived.
$this->audit->deleted(Token::class, $id);

// Something with no PHP class: a projection table, a row in another system, an OLAP fact.
$this->audit->deleted('stripe-subscription', $subscriptionId, ['status' => 'canceled']);
```

Handing it a class is what keeps a manual entry and a captured one in the same history: both land
under that class's type, so `?entityType=token` returns them together. Handing it a string of your own
is how the trail records things the ORM never modelled — the entry simply records no class name, which
`entity_class` is nullable for. See
[attributes.md](attributes.md#the-name-entries-are-filed-under).
