...
Reconstructing that from `entity_type` + `entity_id` means knowing the whole object graph at read
...
(`idx_audit_root (root_type, root_id, occurred_at, id)`), one query at read time.
...
`type` is the short discriminator written to `root_type`. It defaults to the kebab-cased short name
of `root` — `Questionnaire` → `questionnaire`, `SsqPost` → `ssq-post` — and is what you filter on. It
...
columns null, so its own changes are found through `entity_type` + `entity_id` rather than through
...
The walk the bundle performs is structurally query-free. `resolveAuditRoot()` is *yours*, called in
the same place, and nothing stops it from touching an unloaded association — the same contract a
`#[AuditLabel]` method and a `__toString()` are under. Return what is already in memory, or return
null.
...
`AuditLogRepository` returns `QueryBuilder`s ordered by `(occurredAt DESC, id DESC)` — newest first,
and the key every index ends with. The identifier is a UUID v7 and therefore chronological on its
own, right up until something is backfilled, which is why the order and the cursor carry both.
...
| Method | Feed |
| --- | --- |
| `forRoot(string $rootType, string $rootId, ?AuditCursor $before = null)` | one aggregate |
| `forEntity(string $entityTypeOrClass, string $entityId, ?AuditCursor $before = null)` | one row |
| `forActor(?string $actorType, ?string $actorId, ?AuditCursor $before = null)` | one principal (null = unconstrained) |
| `createFeedQueryBuilder(?AuditCursor $before = null)` | everything |

Each takes an `AuditCursor`: `AuditCursor::of($lastEntryDisplayed)` gets you the next page. There is no offset paging and no total count, on purpose — see
...
```
GET /audit-logs?rootType=questionnaire&rootId=0197f1c2-...&itemsPerPage=20
```

and the next page follows the cursor the response advertises, or you build it yourself:

```
GET /audit-logs?rootType=questionnaire&rootId=0197f1c2-...&id[lt]=<last id you rendered>
```
...
For the repository's full surface — the four query builders, cursor paging, custom criteria and
exports — see [reading-the-trail.md](reading-the-trail.md).
