...
doctrine:
    orm:
        entity_managers:
            default:                      # your default manager, or the first one you declare
                mappings:
                    AuditTrail:
                        type: attribute
                        is_bundle: false
...
CREATE INDEX idx_audit_entity   ON audit_logs (entity_class, entity_id, occurred_at);
CREATE INDEX idx_audit_root     ON audit_logs (root_type, root_id, occurred_at);
CREATE INDEX idx_audit_actor    ON audit_logs (actor_type, actor_id, occurred_at);
