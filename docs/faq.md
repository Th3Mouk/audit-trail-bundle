...
No, and the reason is structural rather than a benchmark: **capture issues no database query**. It
reads the change set, the identity map and already-initialised properties, and nothing else. Every
resolver returns `null` rather than doing I/O.
...
DELETE FROM audit_logs
 WHERE occurred_at < now() - INTERVAL '18 months'
   AND entity_class NOT IN ('App\Entity\Contract', 'App\Entity\Payment');
```
