...
The API Platform bridge is off by default, and turning it on requires you to declare
`audit_trail.bridges.api_platform.access` — a list of grants, a raw expression, or an explicit
`public: true`. Declaring none of them stops the container from compiling. An audit feed is a map of
who does what and when, so installing a package must never be what publishes it, and the bundle will
not guess a permission name on your behalf. If you do choose `public: true`, the route is as open as
any other route in your application: cover it with a firewall or an `access_control` rule.
