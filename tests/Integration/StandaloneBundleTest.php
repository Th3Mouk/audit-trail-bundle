...
final class StandaloneBundleTest extends IntegrationTestCase
{
    ...
    /**
     * The claim is that no optional integration is present — not that Symfony registers a fixed
     * list of bundles. Symfony 8 adds its own (`ServicesBundle`, `ConsoleBundle`), so asserting the
     * exact list would only pin the framework's internals and break on the next minor.
     */
    public function testTheKernelRegistersNoOptionalIntegration(): void
    {
        $kernel = self::$kernel;
        self::assertNotNull($kernel);

        $registered = array_keys($kernel->getBundles());

        self::assertContains('DoctrineBundle', $registered);
        self::assertContains('AuditTrailBundle', $registered);

        foreach (['ApiPlatformBundle', 'SecurityBundle', 'StofDoctrineExtensionsBundle'] as $optional) {
            self::assertNotContains(
                $optional,
                $registered,
                \sprintf('Capture must work without %s, so this kernel must not register it.', $optional),
            );
        }
    }
